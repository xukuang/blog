...
## 场景二
在使用R时，(.packages(all.available=TRUE))函数可以查看本地下载了哪些R包，当希望查看当前的R环境中都加载了哪些R包可以使用(.packages())函数或者find.package()函数，临时卸载某个R包可以使用函数detach(package:包名)。如要临时卸载dplyr包，则为detach(package:dplyr)。
